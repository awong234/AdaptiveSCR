...
## This dockerfile is first modified by Alec Wong, January 2020, off of the
## nuest/mro-docker Dockerfile. Additions include installation of JAGS, and
## required packages for the Adaptive Sampling simulation study.
RUN useradd docker \
	&& mkdir /home/docker \
...
RUN apt-get update && apt-get install jags -y

## Installation of R packages
RUN R -e "install.packages(c('doParallel', 'fields', 'ggplot2', 'jagsUI', 'digest'))"